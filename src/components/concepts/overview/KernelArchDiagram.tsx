...
function renderKernelArch(
    ...
    const layers = [
        { label: '유저 공간 (User Space)', y: 10, h: 80, hue: 250 },
        { label: '시스템 콜 인터페이스', y: 110, h: 50, hue: 40 },
        { label: '커널 공간 (Kernel Space)', y: 180, h: 220, hue: 280 },
        { label: '하드웨어 (Hardware)', y: 420, h: 80, hue: 145 },
    ]
    ...
    const userNodes: ArchNode[] = [
        { label: '앱 프로세스', sub: 'bash, nginx, python', x: PAD + 20, y: 35, w: 160, h: 44, fill: c.blueFill, stroke: c.blueStroke, textColor: c.blueText, subColor: c.textMuted },
        { label: '시스템 라이브러리', sub: 'glibc, libpthread', x: PAD + 200, y: 35, w: 160, h: 44, fill: c.blueFill, stroke: c.blueStroke, textColor: c.blueText, subColor: c.textMuted },
        { label: 'GUI / 데몬', sub: 'systemd, Xorg', x: PAD + 380, y: 35, w: 140, h: 44, fill: c.blueFill, stroke: c.blueStroke, textColor: c.blueText, subColor: c.textMuted },
        { label: '런타임', sub: 'JVM, Node.js, Go', x: PAD + 540, y: 35, w: 140, h: 44, fill: c.blueFill, stroke: c.blueStroke, textColor: c.blueText, subColor: c.textMuted },
    ]
    userNodes.forEach(drawNode)

    // Syscall interface
    drawNode({ label: 'System Call Table', sub: 'sys_read / sys_write / sys_open / sys_fork / sys_mmap ...', x: PAD + 60, y: 118, w: fullW - 120, h: 34, fill: c.amberFill, stroke: c.amberStroke, textColor: c.amberText, subColor: c.textMuted })

    // Kernel subsystems — 2 rows
    const ksRow1Y = 200
    const ksRow2Y = 290
    const ksH = 70
    const ksGap = 12
...
export function KernelArchDiagram() {
    ...
    return (
        <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950">
            <D3Container renderFn={renderFn} deps={[theme]} height={520} zoomable />
        </div>
    )
}
