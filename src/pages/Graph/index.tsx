...
function buildData(
    ...
    const filteredGlossary = glossary.filter(g =>
        activeCategory ? g.category === activeCategory : true
    )
    if (showGlossary) {
    ...
    if (showTopics) {
        for (let i = 0; i < kernelTopics.length; i++) {
            for (let j = i + 1; j < kernelTopics.length; j++) {
                ...
                if (shared.length > 0) {
                    links.push({ source: `topic:${kernelTopics[i].id}`, target: `topic:${kernelTopics[j].id}`, kind: 'sharedTag' })
                }
            }
        }
    }
...
export default function Graph() {
    ...
    const btnInactive = 'border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-blue-400 dark:hover:border-blue-600'

    return (
        <div className="max-w-6xl mx-auto px-6 py-10">
            {/* Header */}
            <header className="space-y-2 mb-8">
                <p className="text-xs font-mono text-blue-500 dark:text-blue-400 uppercase tracking-widest">개념 지도</p>
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white">커널 개념 그래프</h1>
                <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed">
                    토픽과 용어 간의 연결 관계를 시각화합니다.
                    실선은 공통 태그를 가진 토픽 연결, 점선은 용어의 토픽 참조입니다.
            ...
            <div className="flex flex-wrap gap-2 mb-3">
                ...
                {Object.keys(CAT_LABEL).map(cat => (
                    <button
                        ...
                        style={activeCategory === cat ? { backgroundColor: CAT_COLOR[cat], borderColor: CAT_COLOR[cat] } : undefined}
                    >
                        {CAT_LABEL[cat]}
                    </button>
                ))}
